...
const ForYou = () => {
  ...
  useEffect(() => {
    fetch('https://cms.samespace.com/items/songs')
      ...
      .then(async (data) => {
        // Filter out top tracks to get only non-top tracks
        const nonTopTracks = data.data.filter(song => !song.top_track);

        // Fetch duration for each song
        const songsWithDuration = await Promise.all(nonTopTracks.map(async song => {
